...
|----|-------|---------------------|
| **PR 2 — Metrics core** | Prometheus + kube-state-metrics + node-exporter in a hardened `monitoring` namespace; cAdvisor scrape; least-privilege scrape RBAC | Layer 1 signals **and** the Layer 2 batch-Job signals (via KSM) are collectable; Job `ttlSecondsAfterFinished` set to honour the [queryability contract](#the-queryability-contract-a-design-requirement-for-the-runtime-prs); extended `k8s/validate.py` passes over `monitoring`; render + kubeconform + dry-run green; **no deploy claim** |
