...
| [ADR-037](ADR-037-pipeline-reliability-hardening.md) | Pipeline reliability hardening from failure-test evidence (Sprint 8 PR 13) — implement **only** the fixes the Sprint 8 failure tests justified (bounded in-run MLflow tracking retry via `src/retry.py`, unit-tested), declining others with reasons; healthy run + start-gate regression re-verified on local Docker Desktop Kubernetes | Accepted (implemented) | 2026-08-21 |
