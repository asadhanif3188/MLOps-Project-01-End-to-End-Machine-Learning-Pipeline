...
| 5 | Pytest | `pytest` | Runs the suite using the `pyproject.toml` config (`pythonpath=src`, `testpaths=tests`, strict markers) — identical to a local `make test`. |
...
| 6 | **Docker Build** | Builds the `runtime` (production) target with `docker/build-push-action@v6`, **`push: false`** and **`load: true`** (image loaded locally for validation, never published). Stamps `VCS_REF`/`BUILD_VERSION` build args; caches layers via `type=gha`. |
| 7 | **Build validation** | Runs the freshly built image and asserts its contract: it runs as the **non-root** user (UID `10001`), the core stack (`sklearn`, `mlflow`, `dvc`, `pandas`) imports cleanly, and the `dvc` entrypoint CLI is present. A build that produces an unusable image fails here. |
...
3. **Automated pipeline validation (v3).** Run `dvc status` / a scoped `dvc repro`
   in CI against a small fixture dataset to catch pipeline-definition breakage.
