...
| Observability & monitoring | ✅ **Prometheus** + **Grafana** on EKS; four-layer signal model (Kubernetes platform, pipeline Job, MLflow, PostgreSQL); 8 alert rules (pipeline failure, OOM, MLflow down, etc.); least-privilege NetworkPolicy enforcement; all critical failure scenarios validated with runbook-driven recovery on live EKS (Sprint 8) — [Observability Architecture](observability.md), [Alerting](alerting.md), [Network Policies](network-policies.md), [ADR-028–034](decisions/ADR-028-observability-architecture.md) |
| Supply-chain & security | ✅ Container vulnerability scanning (Trivy, no fixable HIGH/CRITICAL); CycloneDX SBOM per image; immutable ECR digest provenance (git commit → tag → digest → running pod); hardened Pod Security context (non-root, dropped caps, seccomp, no escalation); least-privilege workload identity (Pod Identity); KMS-encrypted S3 + EKS Secrets; NetworkPolicy deny-by-default — [Container Image Scanning](container-image-scanning.md), [Supply-Chain Provenance](supply-chain-provenance.md), [SECURITY.md](../SECURITY.md), [ADR-035–036](decisions/ADR-035-container-image-scanning.md) |
